...
|---|---|---|
| No message on Discord | Missing/Invalid Webhook URL | Verify `DISCORD_WEBHOOK_URL` in Lambda Environment Variables. |
| Alarm stays in OK state | Incorrect Alarm Statistic | Change statistic to **Sum** in CloudWatch Alarm configuration. |
| Manual test works, real alarm fails | Wrong SNS Topic attached | Confirm the alarm references `discord-alerts` under its Actions. |
| Lambda times out | Default 3s execution limit | Increase timeout to **10 seconds** in Lambda configuration. |
| SNS Subscription pending | Missing IAM permissions | Verify the Lambda execution role allows `sns:Subscribe`. |
| No execution logs | CloudWatch Logs blocked | Attach `AWSLambdaBasicExecutionRole` policy to the Lambda IAM role. |
| Discord returns 401/403/404 | Revoked or corrupt URL | Generate a fresh Webhook URL in Discord server settings. |
| Spikes visible, no alarm triggered | Evaluation periods too high | Adjust "Datapoints to alarm" from `5/5` to `1/1` or `1/5`. |
| Metric shows 1 instead of 5 failures | Wrong aggregation method | Switch metric statistic from **Average** or **Minimum** to **Sum**. |
| Email alert works, Discord fails | Broken SNS -> Lambda linkage | Inspect Lambda execution logs in CloudWatch Logs for python tracebacks. |
